...
(* Thickness type *)
type thickness = {mutable t: int}

(* Thickness definitions for thin vs. thick lines button *)
let thin_lines: thickness = {t = 1}
let thick_lines: thickness = {t = 10}
...
type gctx = {
  ...
  thickness: thickness;  (** the current pen thickness*)
  }
...
(** Draw points at a given list of positions with the given gctx *)
let draw_points (g: gctx) (p_list: position list) : unit = 
  set_graphics_state g;
  List.iter (fun a -> draw_point g a) p_list
